...
whitelist_remote_url = 'https://cdn.cashrewards.com/whitelist/network-cash-back-shopping.txt'
#remote_sql_url = 'https://raw.githubusercontent.com/dragredsim/whitelist/master/scripts/cashrewards.sql'
...
cr_whitelist_location = os.path.join(pihole_location, 'cr-whitelist.txt')
...
whitelist_local = set()
whitelist_cr_local = set()
whitelist_old_cr = set()

os.system('clear')
print('\n')
print('This script will download and add domains from the Cashrewards CDN site to whitelist.')
print('\n')
...
if whitelist_str:
    whitelist_remote.update(
		x for x in map(
			str.strip, whitelist_str.splitlines()
		)
		#at this point, we have a Python list, each item contains one line from the CR file
		if x and x[0:3] == '@@||': #keep only the lines starting with @, these are the exception lines in ABP format
			starPoint = x.find('*')
...
if db_exists:
    ...
    try: # Try to create a DB connection
        ...
        user_add = cursor.execute(" SELECT * FROM domainlist WHERE type = 0 AND comment NOT LIKE '%r83m9f%' ")
        ...
        if ilng == True: # Add domains that are missing from new script and not user additions
            ...
            gravityScript_after = cursor.execute(" SELECT * FROM domainlist WHERE type = 0 AND comment LIKE '%r83m9f%' ")
            # fetch all matching entries which will create a tuple for us
    ...
    if whitelist_local:
        print("[i] {} existing whitelists identified".format(
            len(whitelist_local)))
        if os.path.isfile(cr_whitelist_location) and os.path.getsize(cr_whitelist_location) > 0:
            print('[i] Existing cr-whitelist install identified')
            with open(cr_whitelist_location, 'r') as fOpen:
                whitelist_old_cr.update(x for x in map(
                    str.strip, fOpen) if x and x[:1] != '#')

                if whitelist_old_cr:
                    print('[i] Removing previously installed whitelist')
                    whitelist_local.difference_update(whitelist_old_cr)
    ...
    with open(cr_whitelist_location, 'w') as fWrite:
